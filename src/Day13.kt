...
fun main() {
    ...
    measureTimeMillisPrint {
        ...
        val input1 = input.asSequence().filterNot { s -> s.contains("=") }.filter { it.isNotEmpty() }.map { it.split(",")}.map { p -> Point2d(p.first().toInt(), p.last().toInt()) }.toSet()

...
class Literal(bits: String) : Packet(bits) {
    override val sumOfVersions = bits.take(3).toInt(2)

    override val value = bits.drop(6)
        .chunked(5)
...
class Operator(bits: String, private val subs: List<Packet>) : Packet(bits) {
    override val sumOfVersions = bits.take(3).toInt(2) + subs.sumOf { it.sumOfVersions }
...
fun main() {
    measureTimeMillisPrint {
        val d = Day16(readInput("Day16").joinToString(""))
